...
begin
	...
	if exists (select * from alumno a where a.id = id) then
        signal sqlstate '45000'
			set message_text = 'clave duplicada';
	else 
		insert into alumno values (id, nombre, apellido1, apellido2);
        set funciona = 0;
	end if;
...
call insertarAlumno('6', 'sdfg', 'sdfg', 'sdfg', @salida);
select @salida;
select * from alumno;


/*
3. Vamos a crear un conversor de tiempo. Recibirá un tiempo en segundos y se debe
devolver convertido en horas, minutos y segundos. ¿Es mejor usar una función o
un procedimiento?
*/
use test;
delimiter $$
drop procedure conversor$$
create procedure conversor(in segundosInput int, out horas int, out minutos int, out segundos int)
begin
	set horas = 0;
    set minutos = 0;
	set segundos = segundosInput;
...
call conversor(15000, @horas, @minutos, @segundos);
