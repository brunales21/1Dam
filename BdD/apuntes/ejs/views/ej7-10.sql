...
-- 10.2.7
with empleado_update as (
	select salario, comision from empleado where comision is not null
)
select * from empleado_update;

-- 10.2.8
with empleado_con_delets as (
	select * from empleado where id_departamento in (select id_departamento from departamento where nombre_departamento != 'Ventas') and id_trabajo in (select id_trabajo from trabajo where nombre_trabajo != 'Representante de ventas') 
)
select * from empleado_con_delets;
...
with listado_directores_departamentos as (
	select d.id_departamento as id_departamento_que_dirige, d.id_director, e.id_departamento as id_departamento_donde_trabaja from departamento d left join empleado e on d.id_director = e.id_empleado
)
